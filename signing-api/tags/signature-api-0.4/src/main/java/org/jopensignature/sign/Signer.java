...
public interface Signer {
   ...
   * <p>
   * The given {@link SignatureContext} parameter accompanies the signature request, providing
   * necessary informations for the signing process, like the document to sign on, signature
   * properties and so on. Therefore the {@link SignatureContext} have to be present, not
   * <code>null</code> and should provide the necessary informations for a signature retrieval.
   * <p>
   * The {@link SignatureListener} is a callback mechanism to notify the calling application about
   * signing progress and its result. For detailed informations see the javadoc of the
   * {@link SignatureListener} interface.
   * <p>
   * <b>NOTE:</b><br/>
   * Implementations of this method will be blocking until the signing processes has been finished.
   * 
   * @param context the {@link SignatureContext} providing necessary informations for the signing
   *          process
   * @param listener a {@link SignatureListener} to be notified about the signing process and to get
   *          informations about the signing result.
   * @throws IllegalArgumentException if the {@link SignatureContext} parameter is <code>null</code>
   *           or does not provide the informations need to start a signing request.
   * @throws IllegalStateException if the signer implementation could not start the signing process
   *           due to an invalid or illegal state, e.g. the signing pad is not available.
   */
